...
char* USART_Receive_String(int length)
{
    unsigned volatile int i = 0;
    char* string;
    while (i < length){
        *string = USART_Receive_char();
        string++;
        i++;
    }
    return string;
...
char* USART_Obtain(char Array[15], int length)
{
    unsigned int i = 0;
    ...
    while((i <= length))                 //Grab data till the array fills
    {
        ...
        if(Array[i] == '\f')                //If we receive a \r the master wants to end
        {
